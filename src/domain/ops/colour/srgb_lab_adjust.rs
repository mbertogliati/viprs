...
/// # Examples
/// ```ignore
/// use viprs::domain::ops::colour::srgb_lab_adjust::SRgbLabAdjust;
///
...
impl Op for SRgbLabAdjust {
    ...
    fn validate_region_contract(
        ...
        validate_adjust_bands(input_bands, output_bands)
            .map_err(crate::domain::error::ViprsError::from)
    }
...
mod tests {
    ...
    fn chained_adjust(rgb_data: &[u8], scale: f64, offset: f64) -> Vec<u8> {
        let to_lab = SRgbToLab;
        let linear = Linear::<crate::domain::format::F32>::new(scale, offset).unwrap();
        let to_srgb = LabToSRgb;
    ...
    fn execute_region_rejects_tiles_with_fewer_than_three_bands() {
        ...
        assert!(matches!(
            err,
            crate::domain::error::ViprsError::Build(BuildError::InvalidOperationBands {
                op: "SRgbLabAdjust",
                input_bands: 2,
                output_bands: 2,
                expected: "at least 3 bands",
