    ...
    F::Sample: bytemuck::Pod + Copy,
{
    ...
    fn process_region(&self, _state: &mut (), input: &Tile<F>, output: &mut TileMut<F>) {
        ...
        if input.bands == 1 && self.xfac == 2 && std::mem::size_of::<F::Sample>() == 1 {
            ...
            let input_u8 = Tile::<crate::domain::format::U8>::new(input.region, 1, input_data);
            let mut output_u8 =
                TileMut::<crate::domain::format::U8>::new(output.region, 1, output_data);
            ...
            && std::mem::size_of::<F::Sample>() == 1
        {
            let input_data: &[u8] = bytemuck::cast_slice(input.data);
            let output_data: &mut [u8] = bytemuck::cast_slice_mut(output.data);
            let input_u8 = Tile::<crate::domain::format::U8>::new(input.region, 3, input_data);
            let mut output_u8 =
                TileMut::<crate::domain::format::U8>::new(output.region, 3, output_data);
...
impl<F: BandFormat> crate::domain::op::DynOperation for ZoomBridge<F>
where
