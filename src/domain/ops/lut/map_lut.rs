...
use crate::{
    domain::error::ViprsError,
    domain::op::{Op, OperationBridge, PixelLocalOp},
    domain::ops::resample::sample_conv::FromF64,
    domain::{
...
mod tests {
    ...
    fn non_u8_index_families_clip_and_truncate_exactly() {
        ...
        let u32_op =
            MapLut::<crate::domain::format::U32, U8>::from_table(vec![10u8, 20, 30], 1).unwrap();
        assert_eq!(
        ...
        let i32_op =
            MapLut::<crate::domain::format::I32, U8>::from_table(vec![10u8, 20, 30], 1).unwrap();
        assert_eq!(
