...
//!
//! `ToF64` and `FromF64` are defined here (not in `reduceh.rs`) so that
//! `Affine` and future 2-D resample ops can use them without a circular
//! dependency on `reduceh`.
...
///
/// Integer formats clamp to their `[MIN, MAX]` range; float formats cast directly.
/// This is necessary because convolution weights can produce values outside the
/// representable range of the output format (e.g. Lanczos ringing artefacts).
