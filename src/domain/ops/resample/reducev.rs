...
pub(crate) struct ReduceVBridge<F: BandFormat>
where
    F::Sample: bytemuck::Pod + ReduceSample,
{
    inner: crate::domain::op::OperationBridge<ReduceV<F>>,
...
impl<F: BandFormat> crate::domain::op::DynOperation for ReduceVBridge<F>
where
