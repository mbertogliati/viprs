...
/// Element-wise maximum with a synchronized right-hand-side image.
///
...
pub struct MaxPair<F: BandFormat>
where
...
impl<F: BandFormat> MaxPair<F>
where
    F::Sample: PairMinMaxSample,
{
    #[must_use]
    /// Creates a new `MaxPair`.
            ...
            if rhs_bands == output_bands {
                ...
                for (pixel, rhs) in rhs_row.iter().copied().enumerate().take(tile_width) {
                    ...
                    for band in 0..output_bands {
                        ...
                        dst_row[idx] = src_row[idx].s_maxpair(rhs);
                    }
                }
            }
        }
...
impl<F> PixelLocalOp for MaxPair<F>
where
    F: BandFormat,
    F::Sample: PairMinMaxSample,
{
...
mod tests {
    ...
    fn run_u8(op: &MaxPair<U8>, input_data: &[u8], width: u32, height: u32, bands: u32) -> Vec<u8> {
    ...
    proptest! {
        #[test]
        fn maxpair_identity_when_rhs_matches_input(pixels in proptest::collection::vec(any::<u8>(), 1..=128)) {
            let len = pixels.len();
            let op = MaxPair::<U8>::new(pixels.clone(), len as u32, 1);
            let output = run_u8(&op, &pixels, len as u32, 1, 1);
            prop_assert_eq!(output, pixels);
        }

        #[test]
        fn maxpair_profile_matches_reference(
            ...
            let len = left.len();
            let op = MaxPair::<F32>::new(right.clone(), len as u32, 1);
            let output = run_f32(&op, &left, len as u32, 1, 1);
            for ((actual, lhs), rhs) in output.iter().zip(left.iter()).zip(right.iter()) {
                prop_assert_eq!(*actual, lhs.max(*rhs));
            }
        }
    }

    #[test]
    fn maxpair_boundary_prefers_larger_extremes() {
        let op = MaxPair::<U8>::new(vec![0, 255, 0, 255], 4, 1);
        let output = run_u8(&op, &[255, 0, 1, 254], 4, 1, 1);
        assert_eq!(output, vec![255, 255, 1, 255]);
    }

    #[test]
    fn maxpair_single_band_rhs_broadcasts_across_bands() {
        let op = MaxPair::<U8>::new(vec![5, 10], 2, 1);
        let output = run_u8(&op, &[1, 9, 20, 3], 2, 1, 2);
        assert_eq!(output, vec![5, 9, 20, 10]);
    }
}
