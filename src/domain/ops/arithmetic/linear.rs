...
impl LinearKernelU8 {
    ...
    #[must_use]
    pub fn from_linear(linear: &Linear<crate::domain::format::U8>) -> Option<Self> {
...
mod tests {
    ...
    const ALLOC_ENV: &str = "VIPRS_LINEAR_ALLOC_CHILD";
    const ALLOC_CHILD_TEST: &str = "domain::ops::arithmetic::linear::tests::process_region_uses_no_heap_allocations_after_setup_child";

    fn make_region(w: u32, h: u32) -> Region {
        Region::new(0, 0, w, h)
    }

    fn linear_process_region_alloc_stats() -> crate::test_support::AllocStats {
        let op = Linear::<F32>::new(1.5, -2.0).unwrap();
        let region = make_region(8, 4);
        let input_data: Vec<f32> = (0..region.pixel_count())
    ...
    fn process_region_uses_no_heap_allocations_after_setup() {
        ...
        assert_eq!(
            stats.alloc_count, 0,
            "Linear::process_region should be allocation-free after setup: {stats:?}"
        );
    }
    ...
    fn process_region_uses_no_heap_allocations_after_setup_child() {
        ...
        crate::test_support::emit_alloc_stats(linear_process_region_alloc_stats());
    }
    ...
    fn linear_metadata_and_large_exact_u8_rows_cover_runtime_dispatch() {
        ...
        assert_eq!(
            op.node_spec(17, 1),
            crate::domain::op::NodeSpec::identity(17, 1)
        );
        op.start();
