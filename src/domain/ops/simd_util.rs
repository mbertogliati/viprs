...
macro_rules! simd_select {
    ...
    (avx512f => $avx512f_fn:expr, avx2 => $avx2_fn:expr, _ => $scalar_fn:expr $(,)?) => {{
        ...
        $crate::domain::ops::simd_util::select_avx512f_or_avx2(
            level,
            $avx512f_fn,
            $avx2_fn,
            $scalar_fn,
    ...
    (avx2 => $avx2_fn:expr, _ => $scalar_fn:expr $(,)?) => {{
        use $crate::domain::simd::SimdLevel;
        let level = SimdLevel::detect();
        $crate::domain::ops::simd_util::select_avx2(level, $avx2_fn, $scalar_fn)
    }};
    // neon arm only (aarch64)
    (neon => $neon_fn:expr, _ => $scalar_fn:expr $(,)?) => {{
        use $crate::domain::simd::SimdLevel;
        let level = SimdLevel::detect();
        $crate::domain::ops::simd_util::select_neon(level, $neon_fn, $scalar_fn)
    }};
}
