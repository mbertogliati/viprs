...
pub struct Prewitt<F: BandFormat> {
    _format: PhantomData<F>,
}

impl<F: BandFormat> Prewitt<F> {
    #[must_use]
    /// Creates a new `Prewitt`.
...
impl<F: BandFormat> Default for Prewitt<F> {
    fn default() -> Self {
        Self::new()
    }
}

impl<F> Op for Prewitt<F>
where
    F: BandFormat,
    F::Sample: ToF64 + Pod,
{
    ...
    fn process_region(&self, _state: &mut (), input: &Tile<F>, output: &mut TileMut<F32>) {
        ...
        for oy in 0..out_h {
            for ox in 0..out_w {
                ...
                let y = oy + 1;
                for band in 0..bands {
                    let gx = convolve_mask3_at(input, in_w, bands, x, y, band, &PREWITT_X);
                    let gy = convolve_mask3_at(input, in_w, bands, x, y, band, &PREWITT_Y);
...
mod tests {
    ...
    #[test]
    fn prewitt_metadata_expands_input_by_one_pixel() {
        let op = Prewitt::<F32>::default();
