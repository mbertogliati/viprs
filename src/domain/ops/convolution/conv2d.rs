...
mod tests {
    ...
    const ALLOC_ENV: &str = "VIPRS_CONV2D_ALLOC_CHILD";
    const ALLOC_CHILD_TEST: &str = "domain::ops::convolution::conv2d::tests::process_region_uses_no_heap_allocations_after_setup_child";
    ...
    fn conv2d_process_region_alloc_stats() -> crate::test_support::AllocStats {
        ...
        let input_data: Vec<f32> = (0..input_region.pixel_count())
    ...
    /// A 1×1 identity kernel must produce output equal to input (cast to f32).
    #[test]
    fn identity_kernel_copies_input() {
        let op = Conv2d::<F32>::new(identity_kernel()).unwrap();

        // 1×1 identity: input_region == output_region (radius = 0).
        ...
        for (got, expected) in output_data.iter().zip(input_data.iter()) {
            assert!(
                (got - expected).abs() < 1e-6,
                "identity kernel: expected {expected}, got {got}"
            );
        }
    }

    /// A 3×3 box filter on a uniform image must return the same uniform value.
    #[test]
    fn box_filter_uniform_image() {
        let op = Conv2d::<F32>::new(box_3x3_kernel()).unwrap();

        // For a 4×4 output, input is 6×6 (radius 1 on each side).
        let out_region = Region::new(0, 0, 4, 4);
        let in_region = Region::new(-1, -1, 6, 6);

        // All pixels = 9.0 in the input tile.
        let in_w = 6usize;
        let in_h = 6usize;
        let input_data = vec![9.0f32; in_w * in_h];
        let mut output_data = vec![0.0f32; 4 * 4];
        ...
        for v in &output_data {
            assert!(
                (v - 9.0).abs() < 1e-4,
                "box filter uniform: expected 9.0, got {v}"
            );
        }
    }

    /// `required_input_region` must expand by `radius_x`/`radius_y` on every side.
    #[test]
    fn required_input_region_expands_correctly() {
        let op = Conv2d::<F32>::new(box_3x3_kernel()).unwrap();
        let output = Region::new(0, 0, 4, 4);
        let input = op.required_input_region(&output);
        assert_eq!(input, Region::new(-1, -1, 6, 6));
    }

    #[test]
    fn process_region_uses_no_heap_allocations_after_setup() {
        let stats = crate::test_support::run_alloc_stats_child(ALLOC_CHILD_TEST, ALLOC_ENV);

        assert_eq!(
            stats.alloc_count, 0,
            "Conv2d::process_region should be allocation-free after setup: {stats:?}"
        );
    }

    #[test]
    fn process_region_uses_no_heap_allocations_after_setup_child() {
        if !crate::test_support::should_run_alloc_stats_child(ALLOC_ENV) {
            return;
        }

        crate::test_support::emit_alloc_stats(conv2d_process_region_alloc_stats());
    }
