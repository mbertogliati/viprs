...
/// Binary morphological closing with a flat structuring element.
pub struct Close {
    radius_x: u32,
    radius_y: u32,
    dilate: Dilate,
...
impl Close {
    ...
    pub fn new(mask: Vec<Vec<u8>>) -> Result<Self, &'static str> {
        if mask.is_empty() {
            return Err("Close: mask must not be empty");
        }
        let mask_h = mask.len() as u32;
        let mask_w = mask[0].len() as u32;
        if mask_w == 0 {
            return Err("Close: mask rows must not be empty");
        }
        for row in &mask {
            if row.len() as u32 != mask_w {
                return Err("Close: mask must be rectangular");
            }
            for &value in row {
                if value != 0 && value != 128 && value != 255 {
                    return Err("Close: mask values must be 0, 128, or 255");
                }
            }
        }

        let dilate = Dilate::new(mask.clone())?;
        let erode = Erode::new(mask)?;
    ...
    pub fn rect(n: u32) -> Result<Self, &'static str> {
        if n == 0 {
            return Err("Close::rect: size must be >= 1");
        }
    ...
    fn checked_intermediate_len(region: Region, bands: u32) -> Result<usize, ViprsError> {
        region
            ...
            .ok_or_else(|| ViprsError::ImageTooLarge {
                ...
                details: "close intermediate scratch exceeds addressable memory",
            })
    }
}
...
impl Op for Close {
    ...
    fn process_region(&self, state: &mut Self::State, input: &Tile<U8>, output: &mut TileMut<U8>) {
        ...
        debug_assert_eq!(
            input.region,
            self.dilate.required_input_region(&intermediate_region)
        );
        debug_assert_eq!(
            intermediate_region,
            self.erode.required_input_region(&output.region)
        );

        let Ok(intermediate_len) = Self::checked_intermediate_len(intermediate_region, input.bands)
        else {
            debug_assert!(false, "Close intermediate scratch overflow");
            return;
        };
...

        {
            ...
            self.dilate
                .process_region(&mut state.dilate_state, input, &mut intermediate);
        }

        {
            ...
            self.erode
                .process_region(&mut state.erode_state, &intermediate, output);
        }
    }
}
...
pub(crate) mod tests {
    ...
    pub(crate) fn run_full_image(op: &Close, width: u32, height: u32, data: &[u8]) -> Vec<u8> {
    ...
    #[test]
    fn all_dont_care_close_produces_255() {
        let op = Close::new(vec![vec![128u8, 128u8], vec![128u8, 128u8]]).unwrap();
        assert_eq!(run_full_image(&op, 1, 1, &[0]), vec![255]);
    }

    #[test]
    fn rejects_invalid_masks_and_zero_rect() {
        assert!(Close::new(vec![]).is_err());
        assert!(Close::new(vec![vec![]]).is_err());
        assert!(Close::new(vec![vec![255u8], vec![255u8, 255u8]]).is_err());
        assert!(Close::new(vec![vec![1u8]]).is_err());
        assert!(Close::rect(0).is_err());
    }

    #[test]
    fn close_declares_double_halo() {
        let op = Close::rect(3).unwrap();
    ...
    proptest! {
        #[test]
        fn one_by_one_close_is_identity(
            width in 1u32..=6,
            height in 1u32..=6,
