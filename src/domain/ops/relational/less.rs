...
/// Element-wise strictly-less-than comparison against a scalar.
///
/// Each sample is compared to `rhs`. The output is `255` if the sample
/// is strictly less than `rhs`, `0` otherwise, matching libvips.
pub struct Less<F: NumericBand>
where
    F::Sample: CmpSample,
{
    rhs: F::Sample,
...
/// `Less` is pixel-local: each output sample depends only on the corresponding
/// input sample. See `PixelLocalOp` for invariants.
impl<F> PixelLocalOp for Less<F>
where
    F: NumericBand,
    F::Sample: CmpSample,
{
...
mod tests {
    ...
    #[test]
    fn less_f32_known_values() {
        let op = Less::<F32>::new(5.0);
        let r = make_region(3, 1);
        // 3.0 < 5.0 → true, 5.0 < 5.0 → false, 7.0 < 5.0 → false
    ...
    #[test]
    fn less_boundary_equal_is_false() {
        let op = Less::<F32>::new(5.0);
    ...
    #[test]
    fn less_reports_thin_strip_and_passthrough_region() {
        let op = Less::<F32>::new(5.0);
    ...
    #[test]
    fn less_multiband_processes_each_band_independently() {
        let op = Less::<F32>::new(5.0);
        ...
        assert_eq!(output_data, vec![255u8, 0, 0, 255, 0, 255]);
    }

    proptest! {
        #[test]
        fn less_output_is_always_true_or_false(
            pixels in proptest::collection::vec(0.0f32..=100.0f32, 1..=64),
            rhs in 0.0f32..=100.0f32,
        ) {
            let len = pixels.len();
            let op = Less::<F32>::new(rhs);
