...
/// Element-wise strictly-greater-than comparison against a scalar.
///
/// Each sample is compared to `rhs`. The output is `255` if the sample
/// is strictly greater than `rhs`, `0` otherwise, matching libvips.
pub struct More<F: NumericBand>
where
    F::Sample: CmpSample,
{
    rhs: F::Sample,
...
/// `More` is pixel-local: each output sample depends only on the corresponding
/// input sample. See `PixelLocalOp` for invariants.
impl<F> PixelLocalOp for More<F>
where
    F: NumericBand,
    F::Sample: CmpSample,
{
...
mod tests {
    ...
    #[test]
    fn more_f32_known_values() {
        let op = More::<F32>::new(5.0);
        let r = make_region(3, 1);
        // 3.0 > 5.0 → false, 5.0 > 5.0 → false, 7.0 > 5.0 → true
        ...
        assert_eq!(output_data, vec![0u8, 0, 255]);
    }

    #[test]
    fn more_boundary_equal_is_false() {
        let op = More::<F32>::new(5.0);
    ...
    #[test]
    fn more_reports_thin_strip_and_passthrough_region() {
        let op = More::<F32>::new(5.0);
    ...
    #[test]
    fn more_multiband_processes_each_band_independently() {
        let op = More::<F32>::new(5.0);
        let region = make_region(2, 1);
        let input_data = vec![4.0f32, 5.0, 6.0, 2.0, 7.0, 3.0];
        ...
        assert_eq!(output_data, vec![0u8, 0, 255, 0, 255, 0]);
    }

    proptest! {
        #[test]
        fn more_output_is_always_true_or_false(
            pixels in proptest::collection::vec(0.0f32..=100.0f32, 1..=64),
            rhs in 0.0f32..=100.0f32,
        ) {
            let len = pixels.len();
            let op = More::<F32>::new(rhs);
        ...
        #[test]
        fn more_matches_strict_scalar_comparison(
