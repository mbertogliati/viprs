...
use crate::{
    ...
    domain::ops::relational::CmpSample,
    domain::{
...
///
/// Each sample is compared to `rhs`. The output is `255` if not equal and `0`
/// otherwise, matching libvips relational semantics.
pub struct NotEqual<F: NumericBand>
where
    F::Sample: CmpSample,
{
    rhs: F::Sample,
    ...
    F::Sample: CmpSample,
{
    ...
    fn process_region(&self, _state: &mut (), input: &Tile<F>, output: &mut TileMut<U8>) {
        ...
        for (s, d) in input.data.iter().zip(output.data.iter_mut()) {
            *d = if *s == rhs { 0 } else { u8::MAX };
        }
    }
}

/// `NotEqual` is pixel-local: each output sample depends only on the corresponding
/// input sample. See `PixelLocalOp` for invariants.
impl<F> PixelLocalOp for NotEqual<F>
where
    F: NumericBand,
    F::Sample: CmpSample,
{
...
mod tests {
    ...
    #[test]
    fn not_equal_f32_known_values() {
        let op = NotEqual::<F32>::new(5.0);
        ...
        assert_eq!(output_data, vec![255u8, 0, 255]);
    }

    #[test]
    fn not_equal_all_match_is_all_false() {
        let op = NotEqual::<F32>::new(1.0);
        let r = make_region(3, 1);
        let input_data = vec![1.0f32, 1.0, 1.0];
    ...
    #[test]
    fn not_equal_reports_thin_strip_and_passthrough_region() {
        let op = NotEqual::<F32>::new(5.0);
    ...
    #[test]
    fn not_equal_multiband_processes_each_band_independently() {
        let op = NotEqual::<F32>::new(5.0);
        ...
        assert_eq!(output_data, vec![0u8, 255, 0, 255, 0, 255]);
    }

    #[test]
    fn not_equal_uses_exact_float_equality() {
        let rhs = 1.0f32;
        let almost_equal = f32::from_bits(rhs.to_bits() + 1);
        let op = NotEqual::<F32>::new(rhs);
        let region = make_region(2, 1);
        ...
        assert_eq!(output_data, vec![0u8, 255]);
    }

    proptest! {
        #[test]
        fn not_equal_output_is_always_true_or_false(
        ...
        #[test]
        fn not_equal_matches_exact_scalar_comparison(
