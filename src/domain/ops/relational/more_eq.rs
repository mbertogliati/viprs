...
/// is greater than or equal to `rhs`, `0` otherwise, matching libvips.
pub struct MoreEq<F: NumericBand>
where
    F::Sample: CmpSample,
{
    rhs: F::Sample,
...
impl<F: NumericBand> MoreEq<F>
where
    F::Sample: CmpSample,
{
    /// Creates a new `MoreEq`.
...
/// `MoreEq` is pixel-local: each output sample depends only on the corresponding
/// input sample. See `PixelLocalOp` for invariants.
impl<F> PixelLocalOp for MoreEq<F>
where
    F: NumericBand,
    F::Sample: CmpSample,
{
...
mod tests {
    ...
    #[test]
    fn more_eq_f32_known_values() {
        let op = MoreEq::<F32>::new(5.0);
        let r = make_region(3, 1);
        // 3.0 >= 5.0 → false, 5.0 >= 5.0 → true, 7.0 >= 5.0 → true
        ...
        assert_eq!(output_data, vec![0u8, 255, 255]);
    }

    #[test]
    fn more_eq_boundary_equal_is_true() {
        let op = MoreEq::<F32>::new(5.0);
    ...
    #[test]
    fn more_eq_reports_thin_strip_and_passthrough_region() {
        let op = MoreEq::<F32>::new(5.0);
    ...
    #[test]
    fn more_eq_multiband_processes_each_band_independently() {
        let op = MoreEq::<F32>::new(5.0);
        ...
        assert_eq!(output_data, vec![0u8, 255, 255, 0, 255, 255]);
    }

    proptest! {
        #[test]
        fn more_eq_output_is_always_true_or_false(
            pixels in proptest::collection::vec(0.0f32..=100.0f32, 1..=64),
            rhs in 0.0f32..=100.0f32,
        ) {
            let len = pixels.len();
            let op = MoreEq::<F32>::new(rhs);
        ...
        #[test]
        fn more_eq_matches_scalar_comparison(
