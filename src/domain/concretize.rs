...
mod tests {
    ...
    use crate::domain::ops::point::{Abs, Clamp, Invert, Linear};

    #[test]
    fn identity_is_noop() {
        let chain = ();
        let mut pixels: Vec<u8> = vec![10, 20, 30, 255];
    ...
    fn identity_f32() {
        let chain = ();
        let mut pixels: Vec<f32> = vec![0.1, 0.5, 1.0];
        apply_chain_to_slice::<F32, _>(&chain, &mut pixels);
        assert_eq!(pixels, vec![0.1, 0.5, 1.0]);
    }

    #[test]
    fn tuple_chain_compiles() {
        let chain = ((), ());
        let mut pixels: Vec<u8> = vec![42];
        apply_chain_to_slice::<U8, _>(&chain, &mut pixels);
        assert_eq!(pixels, vec![42]);
    }

    #[test]
    fn deep_tuple_chain_compiles() {
        let chain = ((((), ()), ()), ());
        let mut pixels: Vec<u16> = vec![1000, 2000];
        apply_chain_to_slice::<U16, _>(&chain, &mut pixels);
        assert_eq!(pixels, vec![1000, 2000]);
    }
    ...
    fn src_dst_helper() {
        ...
        apply_chain_src_dst::<U8, _>(&chain, &src, &mut dst);
        assert_eq!(dst, vec![1, 2, 3, 4]);
    }

    // ─── Fusion composition tests ───────────────────────────────────────────

    #[test]
    fn fused_invert_linear_u8() {
        // invert(128) = 127, linear(127, 2.0, -0.5) = 253.5 → 253
        let chain = (Invert, Linear::new(2.0, -0.5));
        let mut pixels: Vec<u8> = vec![128];
        apply_chain_to_slice::<U8, _>(&chain, &mut pixels);
        assert_eq!(pixels, vec![253]);
    }

    #[test]
    fn fused_three_ops_u8() {
        // invert(100) = 155, linear(155, 1.0, -55.0) = 100, clamp(100, 50, 120) = 100
        let chain = ((Invert, Linear::new(1.0, -55.0)), Clamp::new(50.0, 120.0));
        let mut pixels: Vec<u8> = vec![100];
        apply_chain_to_slice::<U8, _>(&chain, &mut pixels);
        assert_eq!(pixels, vec![100]);
    }

    #[test]
    fn fused_four_ops_u8() {
        // invert(0) = 255, linear(255, 0.5, 0) = 127, abs(127) = 127, clamp(127, 0, 100) = 100
        let chain = (
            ((Invert, Linear::new(0.5, 0.0)), Abs),
            Clamp::new(0.0, 100.0),
        );
        let mut pixels: Vec<u8> = vec![0];
        apply_chain_to_slice::<U8, _>(&chain, &mut pixels);
        assert_eq!(pixels, vec![100]);
    }

    #[test]
    fn fused_invert_linear_f32() {
        // invert(0.5) = 0.5, linear(0.5, 2.0, -0.5) = 0.5
        let chain = (Invert, Linear::new(2.0, -0.5));
    ...
    #[test]
    fn same_pipeline_different_formats() {
        // Same chain works for u8 and f32
        let chain = (Invert, Linear::new(1.0, 0.0));

        let mut u8_pixels: Vec<u8> = vec![100];
        apply_chain_to_slice::<U8, _>(&chain, &mut u8_pixels);
        assert_eq!(u8_pixels, vec![155]); // 255 - 100

        let mut f32_pixels: Vec<f32> = vec![0.25];
        apply_chain_to_slice::<F32, _>(&chain, &mut f32_pixels);
        assert!((f32_pixels[0] - 0.75).abs() < 1e-6); // 1.0 - 0.25
    }

    #[test]
    fn chain_builder_ergonomic() {
        use super::Chain;
        ...
        apply_chain_to_slice::<U8, _>(&chain, &mut pixels);
        // 100 → invert → 155 → linear(2,-10) → 300 → clamp(0,200) → 200
        // 200 → invert → 55 → linear(2,-10) → 100 → clamp(0,200) → 100
        assert_eq!(pixels, vec![200, 100]);
    }

    #[test]
    fn chain_implements_concretize() {
        use super::Chain;

        let chain = Chain::new().then(Invert);
        // Can be used anywhere Concretize is expected
        let result = chain.apply_sample::<U8>(100u8);
