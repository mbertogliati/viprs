mod chaos_monkey_12 {
    use bytemuck::Pod;
    use viprs::{
        BuildError, CompiledPipeline, F32, ImageMetadata, InMemoryImage, Interpretation,
        OperationBridge, RecombOp, U8,
        adapters::{
    ...
    #[test]
    fn thumbnail_then_colourspace_to_lab_updates_metadata_after_resize() {
        let image = patterned_rgb_u8(800, 600);

        let (pipeline, output) = execute_to_image_with_output::<U8, F32, _>(&image, |builder| {
            builder
                .with_colorspace(viprs::domain::colorspace::ColorspaceId::SRgb)
                .thumbnail_with(thumbnail(400))?
                .colourspace::<Lab>()
        })
        .expect("thumbnail then Lab conversion should succeed");

        assert_eq!((pipeline.width, pipeline.height), (400, 300));
