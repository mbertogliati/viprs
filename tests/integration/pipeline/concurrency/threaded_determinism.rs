mod chaos_monkey_8 {
    ...
    #[cfg(feature = "png")]
    #[test]
    fn identical_pipelines_are_deterministic_across_threads() {
        let image = Arc::new(patterned_u8(37, 23, 3));
        ...
        let first = handles
            .remove(0)
            .join()
            .expect("thread must finish without panic");

        for handle in handles {
            let output = handle.join().expect("thread must finish without panic");
            assert_eq!(output, first);
        }
    }

    #[test]
    fn histogram_then_source_pipeline_reuse_is_stable() {
        let image = patterned_u8(12, 8, 3);
        let region = Region::new(0, 0, image.width(), image.height());
        let tile = Tile::<U8>::new(region, image.bands(), image.pixels());
        let reducer = HistFindOp::for_format(image.bands(), None, u8::MAX as u32);
        let partial = reducer.reduce_tile(&tile, &region);
        let histogram = <HistFindOp as TileReducer<U8>>::finalize(&reducer, partial);

        assert_eq!(
            histogram.total(),
            (image.width() * image.height() * image.bands()) as u64
        );
