mod robustez_dims {
    ...
    fn execute_without_panicking<FIn, FOut, S: viprs_runtime::pipeline::internal::CommitPlan>(
        image: &Image<FIn>,
        op_name: &str,
    ...
    fn configure_without_panicking<S: viprs_runtime::pipeline::internal::CommitPlan>(
        image: &Image<U8>,
        op_name: &str,
        configure: impl FnOnce(
            viprs_runtime::pipeline::internal::PipelinePlan,
            ...
            -> Result<viprs_runtime::pipeline::internal::PipelinePlan<S>, BuildError>,
    ) -> Result<viprs_runtime::pipeline::internal::PipelinePlan<S>, ViprsError> {
        let result = catch_unwind(AssertUnwindSafe(|| {
            configure(
                viprs_runtime::pipeline::internal::PipelinePlan::from_source(
                    memory_source_from_image(image)?,
    ...
    fn assert_colour_request_is_typed_or_builds<To: Colorspace>(image: &Image<U8>, label: &str) {
        match configure_without_panicking(image, label, |builder| {
            ...
            Ok(builder) => {
                let built = catch_unwind(AssertUnwindSafe(|| builder.compile()));
    ...
    fn one_by_one_image_ops_return_results_without_panicking() {
        ...
        let (_pipeline, inverted) =
            execute_without_panicking::<U8, U8, _>(&grey, "invert", |builder| {
                builder.plan_invert()
            })
        ...
        let (_pipeline, shrunk) =
            execute_without_panicking::<U8, U8, _>(&grey, "shrink", |builder| {
                builder.plan_shrink(1, 1)
            })
        ...
        let (_pipeline, extracted) =
            execute_without_panicking::<U8, U8, _>(&grey, "extract_area", |builder| {
                builder.plan_extract_area(0, 0, 1, 1)
            })
        ...
        let (_pipeline, embedded) =
            execute_without_panicking::<U8, U8, _>(&grey, "embed", |builder| {
                builder.plan_embed(1, 1, 0, 0, 1, 1, ExtendMode::Black)
            })
        ...
        let (_pipeline, flattened) =
            execute_without_panicking::<U8, U8, _>(&rgba, "flatten", |builder| {
                builder.plan_flatten([0.0, 0.0, 0.0, 1.0])
            })
            ...
            cases
        {
            ...
            let (_pipeline, shrunk) =
                execute_without_panicking::<U8, U8, _>(&image, "shrink", |builder| {
                    builder.plan_shrink(h_shrink, v_shrink)
                })
            ...
            let (_pipeline, reduced) =
                execute_without_panicking::<U8, U8, _>(&image, "reduce", |builder| {
                    builder.plan_reduce(h_reduce, v_reduce, InterpolationKernel::Lanczos3)
                })
                .unwrap_or_else(|error| {
    ...
    fn max_safe_dimensions_survive_a_cheap_operation() {
        ...
        let (_pipeline, output) =
            execute_without_panicking::<U8, U8, _>(&image, "invert", |builder| {
                builder.plan_invert()
            })
    ...
    fn non_power_of_two_dimensions_handle_shrink_reduce_and_thumbnail() {
        for (width, height) in [(777, 333), (1001, 999)] {
            ...
            let (_pipeline, reduced) =
                execute_without_panicking::<U8, U8, _>(&image, "reduce", |builder| {
                    builder.plan_reduce(2.5, 3.0, InterpolationKernel::Lanczos3)
                })
                .unwrap_or_else(|error| {
                    panic!("reduce should succeed for {width}x{height}: {error:?}")
                });
        ...
        for (x, y) in points {
            let (_pipeline, extracted) =
                execute_without_panicking::<U8, U8, _>(&image, "extract_area", |builder| {
                    builder.plan_extract_area(x, y, 1, 1)
                })
                .unwrap_or_else(|error| {
    ...
    fn very_large_reduce_factor_collapses_to_one_pixel() {
        ...
        let (_pipeline, output) =
            execute_without_panicking::<U8, U8, _>(&image, "reduce", |builder| {
                builder.plan_reduce(100.0, 100.0, InterpolationKernel::Lanczos3)
            })
    ...
    fn ten_shrink_two_steps_end_at_one_pixel() {
        ...
        let (_pipeline, output) =
            execute_without_panicking::<U8, U8, _>(&image, "shrink-chain", |builder| {
                ...
                for _ in 0..10 {
                    builder = builder.plan_shrink(2, 2)?;
                }
                Ok(builder)
            })
