    ...
    fn embed_background_fill_applies_requested_colour() {
        ...
        let (_pipeline, output) = execute_to_image::<U8, U8, _>(&image, |builder| {
            builder.embed(
                ...
                image.width(),
                image.height(),
                ExtendMode::Background(vec![17.0, 33.0, 201.0]),
            )
        })
        .expect("embed with background should succeed");

        assert_eq!((output.width(), output.height(), output.bands()), (4, 4, 3));

        for &(x, y) in &[(0usize, 0usize), (3, 0), (0, 3), (3, 3)] {
            let idx = (y * output.width() as usize + x) * 3;
            assert_eq!(&output.pixels()[idx..idx + 3], &[17, 33, 201]);
        }
    }
}
