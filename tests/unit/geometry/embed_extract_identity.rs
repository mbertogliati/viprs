mod chaos_monkey_2 {
    ...
    use viprs::{
        BandFormat, BandFormatId, BuildError, CompiledPipeline, F32, ImageMetadata, InMemoryImage,
        Interpretation, U8,
        adapters::{
    ...
    #[test]
    fn pass1_embed_then_extract_is_identity_for_rectangular_rgb() {
        ...
        let (_pipeline, output) = execute_same_format(&image, |builder| {
            builder
                ...
                .extract_area(2, 3, image.width(), image.height())
        })
        .expect("embed then extract should roundtrip");
