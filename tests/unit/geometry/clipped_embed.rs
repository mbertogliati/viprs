mod chaos_monkey_9 {
    ...
    #[cfg(feature = "png")]
    #[test]
    fn embed_larger_source_at_origin_clips_to_destination() {
        let image = patterned_rgb_u8(4, 4);
        let (_pipeline, output) = run_pipeline(&image, |builder| {
            builder.plan_embed(2, 2, 0, 0, image.width(), image.height(), ExtendMode::Black)
        })
        .unwrap();
        ...
        assert_eq!(
            output.pixels(),
            &[19, 7, 191, 36, 18, 196, 32, 36, 210, 49, 47, 215]
        );
    }
}
