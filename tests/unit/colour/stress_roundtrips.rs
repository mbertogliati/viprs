mod chaos_monkey_3 {
    ...
    fn assert_identity_sizes(
        configure: impl Copy + Fn(PipelineBuilder, u32, u32) -> Result<PipelineBuilder, BuildError>,
        tolerance: u8,
    ) {
        for image in [
    ...
    #[test]
    fn pass1_srgb_lab_srgb_roundtrip_is_near_identity_across_extreme_sizes() {
    ...
    #[test]
    fn pass1_srgb_hsv_srgb_roundtrip_is_near_identity_across_extreme_sizes() {
        assert_identity_sizes(
            |builder, _width, _height| {
                builder
                    .with_colorspace(ColorspaceId::SRgb)
    ...
    proptest! {
        ...
        #[test]
        fn pass1_srgb_lab_srgb_roundtrip_random_pixels_stays_within_u8_tolerance(
            pixels in prop::collection::vec(any::<u8>(), 100 * 100 * 3)
        ) {
            let image: Image<U8> = Image::from_buffer(100, 100, 3, pixels)
                .unwrap()
                .with_metadata(rgb_metadata());
            let (_pipeline, output) = execute_to_image(&image, |builder| {
                builder
    ...
    #[test]
    fn pass5_chained_srgb_hsv_srgb_then_identity_ops_stays_near_original() {
        ...
        .expect("chained colourspace pipeline should succeed");
