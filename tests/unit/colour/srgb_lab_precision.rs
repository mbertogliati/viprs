mod chaos_monkey_5 {
    ...
    #[test]
    fn colourspace_srgb_lab_srgb_roundtrip_stays_within_one_unit() {
        for (width, height) in [(256, 256), (1, 8192), (8192, 1)] {
            let image = palette_rgb_u8(width, height);
            ...
            assert_u8_pixels_within_tolerance(image.pixels(), output.pixels(), 1);
        }
    }
}
