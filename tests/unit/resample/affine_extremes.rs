mod chaos_monkey_15 {
    ...
    #[test]
    #[ignore = "BUG: affine panics on very large finite matrix values (B-326)"]
    fn affine_with_huge_scale_stays_finite_for_f32() {
        ...
        .expect("huge affine should not panic");

        assert!(output.pixels().iter().all(|value| value.is_finite()));
    }
    ...
    fn shrink_one_is_exact_identity() {
        ...
        let (pipeline, output) =
            execute_to_image::<U8, U8, _>(&image, |builder| builder.plan_shrink(1, 1))
    ...
    fn thumbnail_of_1x1_image_does_not_upscale() {
        ...
        let (pipeline, output) =
            execute_to_image::<U8, U8, _>(&image, |builder| builder.plan_thumbnail(thumbnail(100)))
