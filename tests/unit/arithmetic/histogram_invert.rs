mod chaos_monkey_18 {
    ...
    #[test]
    fn hist_equal_then_invert_produces_valid_nonempty_output() {
        ...
        assert_ne!(inverted.pixels(), image.pixels());
    }

    #[test]
    #[ignore = "BUG: recomb accepts a 0x3 matrix and materializes a zero-band image instead of returning a typed error"]
    fn zero_band_recomb_returns_typed_error() {
        ...
        assert!(
            result.is_err(),
            "zero-band recomb must fail with a typed error instead of succeeding"
        );
    }
}
