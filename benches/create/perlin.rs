...
use viprs::{
    ...
    domain::{format::F32, op::OperationBridge, ops::create::PerlinOp},
    ports::scheduler::TileScheduler,
};

fn bench_perlin(c: &mut Criterion) {
    let mut group = c.benchmark_group("create_perlin_f32");
            ...
            b.iter(|| {
                ...
                let pipeline = PipelineBuilder::from_source(source)
                    .then(Box::new(OperationBridge::new_pixel_local(
                        PerlinOp::new(size, size, 128, 42).unwrap(),
                        1,
                    )))
                    .unwrap()
                    .build()
...
criterion_group!(benches, bench_perlin);
