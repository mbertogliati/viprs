...
// while `LrMosaicOp` needs two synthetic inputs plus tie-point refinement before merge.
fn bench_lrmosaic(c: &mut Criterion) {
    let mut group = c.benchmark_group("lrmosaic_u8");

    for &size in &common::STANDARD_SIZES {
        let shift_x = (size / 4).max(BLEND_WIDTH);
        let shift_y = (size / 32).max(1);
        ...
        let (_, warm_merge) = op
            ...
            .unwrap_or_else(|err| panic!("lrmosaic warmup failed: {err}"));
        let output_region =
        ...
        group.bench_with_input(BenchmarkId::from_parameter(size), &size, |b, _| {
            b.iter(|| {
                ...
                let (_, merge) = op
                    .detect_and_build_merge(&reference_tile, &secondary_tile)
                    .unwrap_or_else(|err| panic!("lrmosaic benchmark failed: {err}"));
                let mut state = ();
...
criterion_group!(benches, bench_lrmosaic);
