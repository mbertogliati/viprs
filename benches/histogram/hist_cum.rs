...
fn make_pixels(size: u32) -> Vec<u8> {
    ...
    (0..pixel_count)
        .map(|idx| (((idx * 29) + (idx / size as usize) * 11) % 256) as u8)
        .collect()
}
...
fn bench_hist_cum(c: &mut Criterion) {
    let mut group = c.benchmark_group("hist_cum");
        ...
        group.bench_with_input(BenchmarkId::from_parameter(size), &size, |b, &size| {
            b.iter(|| {
                ...
                let pipeline = PipelinePlan::from_source(source)
                    ...
                    .unwrap()
                    .append_dyn_op(Box::new(OperationBridge::new(HistCumOp::<U8>::new(), 1)))
                    .unwrap()
                    .compile()
                    .unwrap();
...
criterion_group!(benches, bench_hist_cum);
