...
use viprs::{
    ...
    domain::{
        ...
        op::OperationBridge,
        ops::{arithmetic::linear::Linear, histogram::HistEntropyOp},
...
fn make_pixels(size: u32) -> Vec<u8> {
    ...
    (0..pixel_count)
        .map(|idx| (((idx * 31) + (idx / size as usize) * 9) % 256) as u8)
        .collect()
}
...
fn bench_hist_entropy(c: &mut Criterion) {
    let mut group = c.benchmark_group("hist_entropy");
...
criterion_group!(benches, bench_hist_entropy);
