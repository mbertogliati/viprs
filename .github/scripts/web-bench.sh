...
if ! $XTASK web-bench "$FIXTURE" -s thumbnail-bytes -n 20 --json   > /tmp/bench-results/web_thumbnail_bytes.json 2>/dev/null; then
  echo "::warning::web-bench failed: thumbnail-bytes"
  FAILED+=("thumbnail-bytes")
fi

echo "=== pipeline-bytes (thumbnail + sharpen + linear → JPEG) ==="
if ! $XTASK web-bench "$FIXTURE" -s pipeline-bytes -n 20 --json   > /tmp/bench-results/web_pipeline_bytes.json 2>/dev/null; then
  echo "::warning::web-bench failed: pipeline-bytes"
  FAILED+=("pipeline-bytes")
fi

echo "=== concurrent (parallel requests: 2, 4, 8, 16 threads) ==="
if ! $XTASK web-bench "$FIXTURE" -s concurrent --concurrency 2,4,8,16 -n 20 --json   > /tmp/bench-results/web_concurrent.json 2>/dev/null; then
...
if [ -f "$LARGE" ]; then
  if ! $XTASK web-bench "$LARGE" -s large-upload -n 10 --json     > /tmp/bench-results/web_large_upload.json 2>/dev/null; then
