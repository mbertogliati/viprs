...
fn patterned_rgba_u8(width: u32, height: u32) -> Image<U8> {
    ...
    Image::from_buffer(width, height, 4, pixels)
        .unwrap()
...
fn rgba_u16_thumbnail_builds_and_executes() {
    ...
    let image = Image::<U16>::from_buffer(width, height, bands, pixels).unwrap();
