...
const SCHARR_X: [[f64; 3]; 3] = [[-3.0, 0.0, 3.0], [-10.0, 0.0, 10.0], [-3.0, 0.0, 3.0]];
const SCHARR_Y: [[f64; 3]; 3] = [[-3.0, -10.0, -3.0], [0.0, 0.0, 0.0], [3.0, 10.0, 3.0]];

/// Scharr gradient magnitude.
pub struct Scharr<F: BandFormat> {
    _format: PhantomData<F>,
}

impl<F: BandFormat> Scharr<F> {
    #[must_use]
    /// Creates a new `Scharr`.
...
impl<F: BandFormat> Default for Scharr<F> {
    fn default() -> Self {
        Self::new()
    }
}

impl<F> Op for Scharr<F>
where
    F: BandFormat,
    F::Sample: ToF64 + Pod,
{
    ...
    fn process_region(&self, _state: &mut (), input: &Tile<F>, output: &mut TileMut<F32>) {
        ...
        for oy in 0..out_h {
            for ox in 0..out_w {
                ...
                let y = oy + 1;
                for band in 0..bands {
                    let gx = convolve_mask3_at(input, in_w, bands, x, y, band, &SCHARR_X);
                    let gy = convolve_mask3_at(input, in_w, bands, x, y, band, &SCHARR_Y);
                    let magnitude = gx.hypot(gy) as f32;
...
mod tests {
    ...
    #[test]
    fn scharr_metadata_expands_input_by_one_pixel() {
        let op = Scharr::<F32>::default();
