...
/// Arc sine of each pixel sample, returning degrees.
...
/// ```
pub struct ASin<F: FloatFormat>(std::marker::PhantomData<F>)
where
    F::Sample: FloatSample;

#[allow(dead_code)]
impl<F: FloatFormat> ASin<F>
where
    F::Sample: FloatSample,
{
    #[must_use]
...
impl<F: FloatFormat> Default for ASin<F>
where
    F::Sample: FloatSample,
{
    fn default() -> Self {
...
impl<F: FloatFormat> Op for ASin<F>
where
    F::Sample: FloatSample,
{
    type Input = F;
...
/// `ASin` is pixel-local: output(x,y) depends only on input(x,y).
/// No neighbourhood access; identity tile geometry. See `PixelLocalOp` for invariants.
impl<F: FloatFormat> PixelLocalOp for ASin<F> where F::Sample: FloatSample {}
#[cfg(test)]
mod tests {
    use super::*;
    ...
    fn asin_f32_known_values() {
        ...
        assert!((output_data[2] - (-90.0)).abs() < 1e-6);
    }

    #[test]
    fn asin_metadata_matches_identity_geometry() {
        let op = ASin::<F32>::default();
    ...
    proptest! {
        #[test]
        fn asin_output_in_valid_range(
            // asin is only defined in [-1, 1]
            ...
            for v in &output {
                prop_assert!(v.is_finite(), "asin output not finite for input in [-1,1]: {}", v);
            }
        }
    }
}
