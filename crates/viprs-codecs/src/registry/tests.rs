...
impl ImageCodec for ExtensionFallbackCodec {
    ...
    fn decode_boxed(
        ...
        let image = InMemoryImage::<U8>::from_buffer(1, 1, 1, vec![src.first().copied().unwrap_or(0)])?;
        Ok(Box::new(image))
    }
...
impl ImageCodec for DirectoryPathCodec {
    ...
    fn decode_boxed_path(
        ...
        assert!(path.is_dir());
        Ok(Box::new(InMemoryImage::<U8>::from_buffer(1, 1, 1, vec![9])?))
    }
...
impl ImageCodec for HeaderSniffCodec {
    ...
    fn decode_boxed(
        ...
        Ok(Box::new(InMemoryImage::<U8>::from_buffer(1, 1, 1, vec![2])?))
    }
...
impl ImageCodec for PathExtensionCodec {
    ...
    fn decode_boxed(
        ...
        Ok(Box::new(InMemoryImage::<U8>::from_buffer(1, 1, 1, vec![1])?))
    }
...
fn registry_and_image_convenience_apis_cover_path_and_error_edges() {
    ...
    let u8_image = InMemoryImage::<U8>::load_with_options(&u8_input, &raw_load_options::<U8>()).unwrap();
