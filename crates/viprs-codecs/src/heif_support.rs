...
mod tests {
    ...
    fn checked_interleaved_helpers_validate_counts_and_overflow() {
        ...
        assert!(matches!(
            sample_err,
            viprs_core::error::ViprsError::ImageTooLarge { details, .. } if details == "sample overflow"
        ));
        ...
        assert!(matches!(
            row_err,
            viprs_core::error::ViprsError::ImageTooLarge { details, .. } if details == "row overflow"
        ));
        ...
        assert!(matches!(
            byte_err,
            viprs_core::error::ViprsError::ImageTooLarge { details, .. } if details == "byte overflow"
        ));
    }
