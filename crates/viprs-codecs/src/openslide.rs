...
fn decode_region_from_slide(
    ...
    validate_output_len::<viprs_core::format::U8>(region, output)?;
...
mod tests {
    ...
    fn test_selected_level_with_zero_width_is_rejected() {
        ...
        assert!(matches!(
            err,
            ViprsError::OpenSlide(viprs_core::error::OpenSlideCodecError::ZeroSizedLevel {
                level: 2,
                width: 0,
                height: 512,
            })
    ...
    fn test_selected_level_with_zero_downsample_is_rejected() {
        ...
        assert!(matches!(
            ...
            Err(ViprsError::OpenSlide(
                viprs_core::error::OpenSlideCodecError::InvalidLevelDownsample {
                    level: 3,
                    downsample,
                }
            )) if downsample == 0.0
    ...
    fn zero_mpp_is_rejected() {
        assert!(matches!(
            ...
            Err(ViprsError::OpenSlide(
                viprs_core::error::OpenSlideCodecError::InvalidMicronsPerPixel {
                    axis,
                    microns_per_pixel,
                }
