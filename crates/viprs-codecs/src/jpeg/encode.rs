...
impl ImageEncoder for JpegCodec {
        ...
        Self: Sized,
    {
        ...
        // `InMemoryImage<F>` and `InMemoryImage<U8>` therefore have identical layouts for this call site.
        let image = normalize_web_output_u8(unsafe {
            &*std::ptr::from_ref(image).cast::<InMemoryImage<U8>>()
        })?;
